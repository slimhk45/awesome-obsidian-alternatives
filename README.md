...
[^2]: The link handler indexes all vault paths, sorted alphanumerically. Links resolve by shortest unique suffix; when multiple files share a name, the first path in sort order wins. Backlinks are rewritten after a path is renamed, and prefixed when shadowed. An alias synced to the base name hides the prefix in reading view.
