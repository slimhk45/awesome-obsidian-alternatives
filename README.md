...
[^2]: The link handler indexes all vault paths alphanumerically and resolves links using path suffixes. Backlinks are automatically updated when paths are renamed. When two suffix homonyms are introduced in the index—by adding or renaming a path—existing links to the lowest-ranking homonym are prefixed to maintain uniqueness. Aliases in sync with the base file name are added to hide the prefix from the reading view.
