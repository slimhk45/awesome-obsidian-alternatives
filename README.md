...
[^2]: The link handler indexes all vault paths alphanumerically and resolves links using the shortest unique suffix. Backlinks are automatically updated when an existing path is renamed. When a higher-ranking homonym is created—by adding or renaming a path—existing links to its lower-ranking homonym are prefixed to maintain uniqueness. To keep the reading view clean, an alias matching the base file name hides the prefix.
