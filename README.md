...
[^2]: The link handler indexes all vault paths alphanumerically and resolves links using any path suffix. Backlinks are automatically updated when paths are renamed. When a higher-ranking suffix homonym is introduced—by adding or renaming a path—existing links to its lower-ranking homonym are prefixed to maintain uniqueness, while an alias kept in-sync with the base file name is added to hide the prefix from the reading view.
